...
class InsertRetur extends StatefulWidget {
  final String username;
  final String idHeaderTrans;
  final String nameBuyer;
   final TabController controller;
  InsertRetur({this.username,this.controller,this.idHeaderTrans,this.nameBuyer});
  @override
...
class _ShowItemPage extends State<InsertRetur>{
    final db= Firestore.instance;
  final Widget child;
...
//ini data yang akan dikirimkan
  ...
  Widget _buildMemberField(data) {
   ...
   Widget _buildTanggalTransField(data) {
    return CupertinoTextField(
      prefix: const Icon(
        Icons.date_range,
        color: CupertinoColors.black,
      ...
      placeholder: 'Date Transaction: $data',
      onChanged: (newName) {
        setState(() {
          // name = newName;
        });
   ...
   Widget _buildTotalField(data) {
    return CupertinoTextField(
      ...
      placeholder: 'ID Transaction: $data',
      onChanged: (newName) {
        setState(() {
          // name = newName;
        });
    ...
    Size screenSize = MediaQuery.of(context).size;
      ...
      children: <Widget>[
       Column(
               children: <Widget>[
                StreamBuilder(
                    stream: db.collection('header_transaksi').document(this.widget.idHeaderTrans).snapshots(),
                    builder:(BuildContext context, snapshot){
                      if(!snapshot.hasData) return const Text('loading..');
                        ...
                        //  headerMemberController = new TextEditingController(text: snapshot.data['nama_member']);
                          ...
                          child: Column(
                            children: <Widget>[
                              _buildMemberField(snapshot.data['nama_member']),
                              // _buildPelayanField(userDocument.documents[0].data['nama_pelayan']),                    
                              _buildTanggalTransField(snapshot.data['tanggal_transaksi']),                 
                              _buildTotalField(this.widget.idHeaderTrans),
                            ],
                          ),
                        );
                      }
             ...
             Column(
               children: <Widget>[
                 Container(
                   ...
                   child:            
                   StreamBuilder<QuerySnapshot>(
                      stream: db.collection('detail_transaksi').where('id_header',isEqualTo: this.widget.idHeaderTrans).snapshots(),
                        ...
                        return new ListView( 
                          children: snapshot.data.documents.map((DocumentSnapshot document) {
                              return CustomListItemTwo(
                                ...
                                quantity:document['qty_beli'],
                                sellingPrice:document['price_pcs'],
                                docID: document['id_header'],
                                idUser: idUser,
                                );
                              }
                            ).toList(),
...
//context2/c yang ada adalah context dr builder yang dibikin
  ...
  void _showDialogSuccess(context) {
   ...
   Firestore.instance.collection('header_transaksi').document(idHeader).get();
    ...
    // });
      ...
      // //untuk neambahkan ke detail transaksi.
      // await Firestore.instance.collection('owner').document(idUser).collection('cart').getDocuments().then((snapshot){
      //   for(DocumentSnapshot ds in snapshot.documents){

      //     Firestore.instance.collection('owner').document(idUser).collection('header_transaksi').document(ref.documentID).collection('detail_transaksi').add({
      //         'id_header': ref,
    ...
    await Firestore.instance.collection('owner').document(idUser).collection('cart_header').getDocuments().then((snapshot){
        for(DocumentSnapshot ds in snapshot.documents){
           Firestore.instance.collection('owner').document(idUser).collection('retur_penjualan').add({
    ...
    return Scaffold(
      ...
      bottomNavigationBar: Container(
        ...
        child: InkWell(
          ...
          onTap: (){ 
            // print(this.widget.nameBuyer);
            queryValues();
            if(totalRetur>0){
              confirmRetur(this.widget.idHeaderTrans,this.widget.nameBuyer);
              print("bener");
            }else{
              print("salah");
            }
